...
- **DR 위임** — 멀티사이트 재해복구·RTO/RPO·백업 주기 등 플랫폼 DR 정책은 [①](https://github.com/JaeHoYun/vcf-private-ai-guide)에 위임합니다. 이 가이드의 몫은 **무엇이 stateful한지 식별**해 DR 범위에서 빠뜨리지 않게 하는 것입니다.
...
- **목표 수준(SLO)** — 가용성·TTFT·오류율 같은 지표에 목표값을 정해 둡니다. 기준선을 실측한 뒤 현실적인 값으로 잡고, 이탈이 잦으면 용량([⑥](https://github.com/JaeHoYun/vcf-private-ai-sizing-cost))이나 설계를 재검토합니다.
- **온콜·에스컬레이션** — 1차 대응은 알려진 이슈·트러블슈팅(§7.4)으로, 해소되지 않으면 플랫폼 운영([①](https://github.com/JaeHoYun/vcf-private-ai-guide))·보안([⑤](https://github.com/JaeHoYun/vcf-private-ai-security-governance))으로 에스컬레이션합니다.
