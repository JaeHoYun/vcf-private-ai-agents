...
> **CLI 주의** — Artifact Mirroring Tool은 별도 미러링 스크립트(`vpaifn-airgap-res-mirror.sh` 계열)로 수행하며, `vcf pais` 하위 명령이 아닙니다. 공식 CLI 레퍼런스에 `vcf pais amt` 같은 명령은 존재하지 않습니다(§5.5). 에어갭 절차는 릴리스마다 달라질 수 있으니 공식 문서·KB를 따르십시오.
