...
flowchart LR
    ...
    subgraph DEC["존 3: 복호화 전처리 (격리)"]
        ...
        D2["문서보안 서버와 키 서버 연동"]
        D3["정규화, 한글 문서 변환, OCR<br/>PII 마스킹, 청킹, 접근 주체 평탄화"]
        D4["스테이징 (S3 호환)"]
    end
...
국내 저장소에는 다른 곳에 없는 장벽이 있습니다. 한글 워드프로세서 형식(HWP, HWPX)은 PAIS 관리형 커넥터의 지원 형식 목록에 없으므로 존 3에서 변환해야 하고, 스캔 문서는 한국어 인식 품질이 검증된 OCR을 골라야 하며, 청킹은 한국어 문장 경계를 인식하는 분리기를 써야 하고, 임베딩은 한국어 검색 품질을 자사 문서로 실측해 골라야 합니다. 같은 내용이라도 한국어는 토크나이저에 따라 영어보다 토큰이 훨씬 많이 들어 컨텍스트 예산과 비용에 영향을 줍니다. 각 선택지와 기준은 [④ 02 2.3.2절](https://github.com/JaeHoYun/vcf-private-ai/blob/main/04-rag/docs/02-ingestion-indexing.md)에, 모델과 임베딩의 선정 기준은 구축 편 [10](10-models-serving.md)에 있습니다.
...
**확인 사항** — PAIS Data Indexing의 문서 단위 권한 필터 유무와 지원 형식(한글 워드프로세서 형식 포함)은 공식 문서 최신판으로 확인합니다. 문서보안 제품의 서버 측 복호화 방식, 허용 프로세스 정책, 키 서버 연동은 제품 문서와 벤더 확인으로 정합니다.
