...
> 번호가 비어 있는 자리(04~07, 11, 12)는 설계 편의 사용자 신원과 권한 전파, 플랫폼 소비(게이트웨이, 온보딩, 토큰 예산), 데이터 소스 온보딩과 보호 문서, 사내 시스템 연동과 쓰기 설계, 구축 편의 앱 통합과 신뢰 UX, 검증 편의 서비스 보안 준비가 들어갈 자리이며 순차로 채웁니다. 번호는 서비스 수명주기 순서를 따르므로 그 사이의 문서가 추가되어도 기존 번호는 바뀌지 않습니다.
